    ...
    public function dashboardUtama()
{
    ...
    {
        // Mengambil data seluruh tambang aktif dari database.
        // ST_AsText() digunakan untuk mengubah objek spasial di MySQL menjadi string teks biasa (WKT) 
        // agar bisa di-parsing oleh JavaScript Leaflet di view.
        $tambangAktif = DB::table('tambang_aktif')
            ->select('nama_perusahaan', 'nomor_iup', DB::raw('ST_AsText(koordinat_wilayah) as area'))
            ->get();

        return view('peta', compact('tambangAktif'));
    }
